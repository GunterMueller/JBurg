...
int main()
{
    ...
    JBurgCppTestEmitter burm;
    burm.burm(&root);
    long result = burm.getlongResult();

    if ( 3 == result )
    {
        std::cerr << "Succeeded: C++ target.\n";
    }
    else
    {
        std::cerr << "IntegerArithmeticMain FAILED: expected 3, actual " << result << std::endl;
        return 1;
    }
    return 0;
}
