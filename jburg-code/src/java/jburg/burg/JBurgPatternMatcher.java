...
class JBurgPatternMatcher implements Comparable<JBurgPatternMatcher>
{
	 ...
	 */
	final private Vector<JBurgPatternMatcher> subPatterns = new Vector<JBurgPatternMatcher>();

    final private Vector<AST> guardFunctions = new Vector<AST>();
	...
	private Object getNaryCost(EmitLang emitter, String stem)
	{
        ...
        Object rootPath = generatePathToRoot(emitter, stem);

        if (getSubgoal().equals(emitter.getConfiguration().wildcardState)) {
            return emitter.getTemplate(
                "subtract",
                "lhs", emitter.genCallMethod(rootPath, "getArity"),
                "rhs", this.positionInParent
            );
        } else {
     ...
     * </ol>
     * @param other - the pattern matcher against which to compare.
     * @return
     */
    @Override
